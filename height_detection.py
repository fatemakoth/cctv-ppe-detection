...
PERSON_CLASS_ID = 0
ALERT_HEIGHT_CM = 185       # head above this = elevation alert
...
def classify(x1, y1, x2, y2, floor_y, pixels_per_cm):
    ...
    # Head height is reliable across distances — perspective shifts both floor_y reference
    # and y1 in the same direction, mostly cancelling out.
    # Foot-based checks require multi-point homography calibration to work at varying depths.
...
def draw_person(frame, x1, y1, x2, y2, info, pid):
    ...
    if info["elevated"]:
        cv2.putText(frame, "!! ELEVATED !!",
                    (x1, y2 + 22), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0, 0, 255), 2)
